...
def Main():
	...
	for stringa in lines:
		...
		if m!=None:
			...
			if campi[1]==' //':
				...
				# controllando se c'e' un corrispettivo nel .mat letto prima ed eventualmente
	...
	# suppongo che le variabili siano ordinate nello stesso modo sia nel file mat che nel file txt
	fid.close()
	fo.close()
...
if time_steps <= 0:
	print "ABORT: Simulation Failed - Problem with OMC"
else:
	...
	for j in range(len(data_name[0])):
		...
		for i in range(len(data_name[:])):
			try:
				...
				# il problema di fatto non sussiste, i nomi vengono
				# memorizzati correttamente e tutto funziona
