// Simulation code for ExternalLibraries generated by the OpenModelica Compiler 1.7.0 (r8719).
...
extern "C" {
#endif
#include "ExternalFunc2.h"
#include "ExternalFunc1_ext.h"
#ifdef __cplusplus
}
#endif
#include "ExternalLibraries_functions.cpp"
...
extern "C" { // adrpo: this is needed for Visual C++ compilation to work!
  const char *model_name="ExternalLibraries";
  const char *model_fileprefix="ExternalLibraries";
...
const struct omc_varInfo state_names[2] = {
  {1000,"x","",{"TestExt.mo",2,2,2,32,false}},
  {1001,"y","",{"TestExt.mo",2,2,2,32,false}}
};
#define $Px__varInfo state_names[0]
#define $Py__varInfo state_names[1]
const struct omc_varInfo derivative_names[2] = {
  {1002,"der(x)","",{"TestExt.mo",2,2,2,32,false}},
  {1003,"der(y)","",{"TestExt.mo",2,2,2,32,false}}
};
...
struct omc_functionInfo function_names[2] = {
  {1004,"ExternalFunc2",{"TestExt.mo",15,1,19,18,false}},
  {1005,"ExternalFunc1",{"TestExt.mo",8,1,13,18,false}}
};
...
#define ExternalFunc2_index 0
#define ExternalFunc1_index 1
...
const struct omc_equationInfo equation_info[4] = {
  {1006,"SES_ALGORITHM 0",0,NULL},
  {1007,"SES_SIMPLE_ASSIGN 1",1,&$P$DER$Py__varInfo},
  {1008,"SES_ALGORITHM 2",0,NULL},
  {1009,"SES_SIMPLE_ASSIGN 3",1,&$P$DER$Px__varInfo}
};
...
{
  state mem_state;
  ExternalFunc2_rettype tmp1012;
  ExternalFunc1_rettype tmp1013;
  
  mem_state = get_memory_state();
  #ifdef _OMC_MEASURE_TIME
  SIM_PROF_TICK_FN(ExternalFunc2_index);
  #endif
  tmp1012 = _ExternalFunc2($Py);
  #ifdef _OMC_MEASURE_TIME
  SIM_PROF_ACC_FN(ExternalFunc2_index);
  #endif
  $P$DER$Py = (-tmp1012.ExternalFunc2_rettype_1); 
  #ifdef _OMC_MEASURE_TIME
  SIM_PROF_TICK_FN(ExternalFunc1_index);
  #endif
  tmp1013 = _ExternalFunc1($Px);
  #ifdef _OMC_MEASURE_TIME
  SIM_PROF_ACC_FN(ExternalFunc1_index);
  #endif
  $P$DER$Px = (-tmp1013.ExternalFunc1_rettype_1); 
  restore_memory_state(mem_state);
...
{
  state mem_state;
  ExternalFunc2_rettype tmp1014;
  ExternalFunc1_rettype tmp1015;
  
  mem_state = get_memory_state();
  begin_inline();
  tmp1014 = _ExternalFunc2($Py);
  $P$DER$Py = (-tmp1014.ExternalFunc2_rettype_1); 
  inline_integrate($P$DER$Py);
  tmp1015 = _ExternalFunc1($Px);
  $P$DER$Px = (-tmp1015.ExternalFunc1_rettype_1); 
...
{
  state mem_state;
  ExternalFunc2_rettype tmp1016;
  ExternalFunc1_rettype tmp1017;
  *needToIterate = 0;
  ...
  SIM_PROF_TICK_FN(ExternalFunc2_index);
  #endif
  tmp1016 = _ExternalFunc2($Py);
  #ifdef _OMC_MEASURE_TIME
  SIM_PROF_ACC_FN(ExternalFunc2_index);
  #endif
  $P$DER$Py = (-tmp1016.ExternalFunc2_rettype_1); 
  #ifdef _OMC_MEASURE_TIME
  SIM_PROF_TICK_FN(ExternalFunc1_index);
  #endif
  tmp1017 = _ExternalFunc1($Px);
  #ifdef _OMC_MEASURE_TIME
  SIM_PROF_ACC_FN(ExternalFunc1_index);
  #endif
  $P$DER$Px = (-tmp1017.ExternalFunc1_rettype_1); 
  restore_memory_state(mem_state);
