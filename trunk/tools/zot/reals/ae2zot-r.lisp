...
(defun dom-evt-futr ()
  (format t "dom-evt-futr...")(force-output)
  (let ((k (kripke-k *PROPS*))) 
	(loop for fm in (kripke-futr *PROPS*) when (member (car fm) '(until release)) collect
	      (case (car fm)
		    ((until)
			  (cons 'or 
				(loop for i from 1 to (kripke-k *PROPS*) collect
				      `(= ,(intern (format nil "I_EVE_~s" (call-fmla-id *PROPS* fm))) ,i))))
		    ((release)	
			  (cons 'or
				(loop for i from 1 to (kripke-k *PROPS*) collect
				      `(= ,(intern (format nil "I_EVE_~s" (call-fmla-id *PROPS* fm))) ,i))))))))
		    ...
		    (let ((*print-case* :downcase))
		      ...
		      (maphash (lambda (key v) 
				 (if (consp key) 
				     (case (car key)
				       ((until release)
					(format k ":extrapreds (( ~s Real ))~%" v)
					(format k ":extrafuns (( i_eve_~s Real ))~%" v))			       
				       (t
					(if (not (arith-itemp key))
					    (format k ":extrapreds (( ~s Real ))~%" v)))) ;legacy for (-P- a)
				     (if (member key (kripke-prop *PROPS*))
					 (format k ":extrapreds (( ~s Real))~%" v)				 
					 (case key
					   ((**LOOPEX**) (format k ":extrapreds (( ~s ))~%" v))
					   ((**I_LOOP**) (format k ":extrafuns (( ~s Real))~%" v))				 
					   (t (format k ":extrapreds (( ~s Real))~%" v))))))
			       (kripke-list *PROPS*))
		      ...
		      (maphash (lambda (key v) 
				   (if (consp key)
				       (case (car key)
					 ((next yesterday)
					  (format k ":extrafuns (( ~s Real Real))~%" v))
					 ((+ - * / mod)
					  (format k ":extrafuns (( ~s Real Real))~%" v))) ))
